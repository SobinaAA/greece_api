...
```typescript
import { AuthApi, Configuration, LoginPostRequest } from './api';
...
```typescript
import { AuthApi, Configuration, RegisterPostRequest } from './api';
...
const { status, data } = await apiInstance.registerPost(registerPostRequest);
```
